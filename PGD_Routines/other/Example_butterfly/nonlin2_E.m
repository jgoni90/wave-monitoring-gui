function f = nonlin2_E(meshes,mode)

% X1,T1,re1:        E mesh
% X2,T2,re2:        THETA mesh
% X3,T3,re3:        D mesh
% S(THETA),T(D)     nodal vectors for THETA and D dimensions to be integrated
% This rotine CANNOT be used also to build the vector for the THETA or D mesh
...
for ie1 = ve1
    ...
    X       = reshape(X1_t(:,Te1),nen1,1,1,nE1,1,1);
    ...
    for ie2 = ve2
        ...
        K       = reshape(X2_t(:,Te2),1,nen2,1,1,nE2,1);
        
        %Jacobian (straight elements!!)
        J2 = (1/2) * abs((K(:,end,:,:,:,:) - K(:,1,:,:,:,:)));
        
        %Nodal coordinates of function S and product with jacobian
        iS = bsxfun(@times,reshape(S(Te2),1,nen2,1,1,nE2,1),J2);
        ...
        for ie3 = ve3
            ...
            COEF = bsxfun(@rdivide,K,X);
            COEF = sin(COEF);
            COEF = bsxfun(@power,COEF,THETA);
            
            %Interpolation of the non-linear function and product with jacobian
            fval = N_g1g2g3_ilm * reshape(COEF,nen1*nen2*nen3,nE1*nE2*nE3);
            ...
            tensor_beta = bsxfun(@times,tensor_beta,reshape(iS,1,ng2,1,1,nE2,1));
