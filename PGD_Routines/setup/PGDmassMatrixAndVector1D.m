function [C,f] = PGDmassMatrixAndVector1D(X,T,theReferenceElement,coefficients,var,elemCoef)
...
for icoef = 1:nOfCoefs
    ...
    f{icoef} = zeros(nOfNodes,nOfTerms);
    I = zeros(NNZ,1);
    J = I;
    ...
    for ielem = 1:nOfElements
        ...
        %Elemental matrices
        [Ce,fe] = elementalMatrix(Xe,theReferenceElement,coefe);
        Ce = alpha*Ce; %alpha is constant element by element
        
        %Assembling
        f{icoef}(Te,:) = f{icoef}(Te,:) + fe;
...
function [Ce,fe] = elementalMatrix(Xe,theReferenceElement,coefe)
...
for g = 1:ngauss
    ...
    elemF = N_g'*dline;
    for i = 1:nOfTerms
        Ce(:,:,i) = Ce(:,:,i) + coef(i)*elemN; 
        fe(:,i) = fe(:,i) + coef(i)*elemF;
    end
end
