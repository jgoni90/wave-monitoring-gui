function projectedpgd = PGDprojection_NonSeparableVector(fhandle,matrices,meshes,rhs,parameters,algorithm)
...
%Is given the function handle, for each dimension, for computing the non-separable term.
...
while ~Uconvergence && projectedpgd.counters.Uterm < parameterscopy.maxterms
    ...
    while ~PGDconvergence && iter < algorithm.projection.maxiter
        ...
        for idim = 1:parameters.nOfPGDdimensions
            ...
            %Vector f
            fcoef = fhandle{idim}(meshes,mode(restdims),matrices);
    ...
    if algorithm.projection.projCoeff
        [projectedpgd.alpha,MatCoeff,fCoeff] = computeProjectionCoefficients_NonSeparableVector...
            (projectedpgd,mode,MatCoeff,fCoeff,matrices,meshes,parameters,fhandle);
    else
...
function [alpha,Mat,f] = computeProjectionCoefficients_NonSeparableVector...
            (pgd,mode,prevMat,prevf,matrices,meshes,parameters,fhandle)
...
%Last term of non-separable RHS vector computed with fhandle
f(Uterm+1) = (mode{1}' * fhandle{1}(meshes,mode(2:parameters.nOfPGDdimensions),matrices));
