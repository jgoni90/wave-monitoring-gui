function f = projectionReUIVectorY_NonSeparable_THETA(meshes,mode)

% X1,T1,re1:        THETA mesh
% X2,T2,re2:        X mesh
% X3,T3,re3:        K mesh
% R(X),S(K):        nodal vectors for X and K dimensions to be integrated
...
T2  = meshes(1).T;
T3  = meshes(2).T;
re1 = meshes(3).referenceElement;
re2 = meshes(1).referenceElement;
re3 = meshes(2).referenceElement;
R   = mode{1};
S   = mode{2};
...
for ie1 = ve1
    ...
    THETA   = reshape(X1_t(:,Te1),nen1,1,1,nE1,1,1);
    
    %Jacobian (straight elements!!)
    J1 = (1/2) * abs((THETA(end,:,:,:,:,:) - THETA(1,:,:,:,:,:)));
    ...
    for ie2 = ve2
        ...
        for ie3 = ve3
            ...
            K       = reshape(X3_t(:,Te3),1,1,nen3,1,1,nE3);
            
            %Jacobian (straight elements!!)
            J3 = (1/2) * abs((K(:,:,end,:,:,:) - K(:,:,1,:,:,:)));
            
            %Nodal coordinates of function T and product with jacobian
            iS = bsxfun(@times,reshape(S(Te3),1,1,nen3,1,1,nE3),J3);
            iS = N_g3_m * reshape(iS,nen3,nE3);
            
            %Incident wave valued at coordinates of the nodes
            UI = bsxfun(@times,K,sin(THETA));
            ...
            tensor_beta = bsxfun(@times,fval,reshape(iS,1,1,ng3,1,1,nE3));
