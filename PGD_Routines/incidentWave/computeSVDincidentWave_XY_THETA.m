function ui = computeSVDincidentWave_XY_THETA(PGDmeshes,parameters)
...
nOfNodes_w = size(PGDmeshes(dimTHETA).X,1);
...
for j = 1:nOfNodes_w
    SVDdata(:,j) = info.handle(PGDmeshes(dimTHETA).X(j),...
                                   bottom,...
                                   PGDmeshes(dimXY).X(xy_nodes,:),...
                                   parameters.fixedParametricDims(1));
end
...
ui.RB{dimTHETA} = conj(V(:,vterms));

%% OTHER FUNCTIONS

function v = myGetField(s,f), v = s; for i = 1:numel(f), v = v.(f{i}); end

function p = uifunction(t,h,X,w)
x = X(:,1);
y = X(:,2);
