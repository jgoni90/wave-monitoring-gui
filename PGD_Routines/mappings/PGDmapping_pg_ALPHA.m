...
if mystrcmp({'XY' 'K' 'THETA'},restnames)
    ...
    for i = 1:coefs(coefsINDEX(end)).nOfTerms
        iRt = Rdt * matrices(dimXY).C_alpha{idim}{i};
        iF  = iRt * pgdRB{dimXY};
    ...
    for i = 1:numel(rhscoefs)
        ...
        iF = 0;
        for j = 1:numel(rhsXY)
            iF = iF + (Rdt*rhsXY{j}) * (STt*rhsKTHETA{j}).';
        end
    ...
    %Compute dual Ps
    iP = matrices(idim).M * P;
    fs = (iALPHAdualcoef * (T'*matrices(dimTHETA).M*T) * (S'*matrices(dimK).M*S) * (R'*matrices(dimXY).M*R))' * iP;
    Ps = lu_P'*(lu_L'\(lu_U'\(lu_Q'*fs)));
...
elseif mystrcmp({'XY' 'K'},restnames) | mystrcmp({'XY' 'KTHETA'},restnames) %#ok<OR2>
    ...
    %Compute dual Ps
    iP = matrices(idim).M * P;
    fs = (iALPHAdualcoef * (S'*matrices(dimK).M*S) * (R'*matrices(dimXY).M*R))' * iP;
    Ps = lu_P'*(lu_L'\(lu_U'\(lu_Q'*fs)));
...
elseif mystrcmp({'XY' 'THETA'},restnames)
    ...
    prevCoefXYTALPHA_4 = iALPHArhscoef * (Rdt*iF) * alphaT.';
    ...
    %Compute dual Ps
    iP = matrices(idim).M * P;
    fs = (iALPHAdualcoef * (T'*matrices(dimTHETA).M*T) * (R'*matrices(dimXY).M*R))' * iP;
    Ps = lu_P'*(lu_L'\(lu_U'\(lu_Q'*fs)));
...
elseif mystrcmp('XY',restnames)
    ...
    for i = 1:numel(rhscoefs)
        ...
        prevCoefXYALPHA_4 = prevCoefXYALPHA_4 + icoef*sum(Rdt*iF);    
    end
    ...
    %Compute dual Ps
    iP = matrices(idim).M * P;
    fs = (iALPHAdualcoef * (R'*matrices(dimXY).M*R))' * iP;
    Ps = lu_P'*(lu_L'\(lu_U'\(lu_Q'*fs)));
end
