...
for nDeg = 0:n
  ...
  for i = 0:nDeg
     if i==0
         ...
         p_i = jacobiP(r,0,0,i); dp_i = jacobiP(r,1,1,i-1)*(i+1)/2;    
     ...
     for j = 0:(nDeg-i)
        if j==0
           ...
           p_j = jacobiP(s,2*i+1,0,j); dp_j = jacobiP(s,2*i+2,1,j-1)*(j+2*i+2)/2;  
        ...
        if k==0
           ...
           p_k = jacobiP(t,2*(i+j)+2,0,k);  dp_k = jacobiP(t,2*(i+j)+3,1,k-1)*(k+2*i+2*j+3)/2;
        end
        ncount= ncount+1;
